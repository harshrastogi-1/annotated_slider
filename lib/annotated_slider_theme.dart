...
class AnnotatedSliderTheme extends InheritedTheme {
  const AnnotatedSliderTheme({
    super.key,
    required this.data,
    required super.child,
  });

  final AnnotatedSliderThemeData data;

  static AnnotatedSliderThemeData of(BuildContext context) {
    final AnnotatedSliderTheme? inheritedTheme =
        context.dependOnInheritedWidgetOfExactType<AnnotatedSliderTheme>();
    return inheritedTheme?.data ?? AnnotatedSliderThemeData.fallback();
  }

  @override
  Widget wrap(BuildContext context, Widget child) {
    return AnnotatedSliderTheme(data: data, child: child);
  }

  @override
  bool updateShouldNotify(AnnotatedSliderTheme oldWidget) =>
      data != oldWidget.data;
}

@immutable
class AnnotatedSliderThemeData with Diagnosticable {
  ...
  factory AnnotatedSliderThemeData.fallback() {
    return const AnnotatedSliderThemeData(
  ...
  static AnnotatedSliderThemeData lerp(
    AnnotatedSliderThemeData a,
    AnnotatedSliderThemeData b,
    double t,
  ) {
    if (identical(a, b)) {
      return a;
    ...
    return AnnotatedSliderThemeData(
      ...
      activeTickMarkColor: Color.lerp(
        a.activeTickMarkColor,
        b.activeTickMarkColor,
        t,
      ),
      inactiveTickMarkColor: Color.lerp(
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      DoubleProperty('trackGap', trackGap, defaultValue: defaultData.trackGap),
    );
    properties.add(
      DiagnosticsProperty<bool>(
        'year2023',
        year2023,
        defaultValue: defaultData.year2023,
      ),
    );
  }
}
  ...
  Rect getPreferredRect({
    ...
    final double trackLeft =
        ...
        (sliderTheme.padding == null
            ? math.max(overlayWidth / 2, thumbWidth / 2)
            : 0);
...
class AnnotatedGappedSliderTrackShape extends AnnotatedSliderTrackShape
  ...
  void paint(
    ...
    final bool drawLeftTrack =
        thumbCenter.dx > (leftRRect.left + (sliderTheme.trackHeight! / 2));
...
class _AnnotatedRoundedRectSliderValueIndicatorPathPainter {
  ...
  double _upperRectangleWidth(TextPainter labelPainter, double scale) {
    final double unscaledWidth =
        math.max(_minLabelWidth, labelPainter.width) + (_labelPadding * 2);
    return unscaledWidth * scale;
  }
